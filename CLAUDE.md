...
design.md EXT-18追記。STOCKの主キー変更は見送りと結論）は全項目完了した。次の一手は特に決まって
...
| ダッシュボード | 遅延ランキング | 高（`domain/schedule.ts`の`checkSchedule()`が返す警告を並べ替えるだけで済む） | 日程整合警告を遅延日数の大きい順にソートして一覧表示する |
| ダッシュボード | アラート件数バッジからのワンクリック遷移 | 高（`TodayActionsBar.tsx`で実装済みの「クリックでタブ遷移」パターンを転用するだけ。現状`DashboardPanel.tsx`のアラート件数バッジに`onClick`は無い） | ダッシュボードのアラート件数バッジをクリックすると該当タブへ遷移できるようにする（現状はAlertBar側の導線のみ） |
| 演習ガイド | 演習完了レポートの出力（新規提案） | 高（`exerciseGuide.ts`の`computeGuideProgress()`が既にTC-01〜18の完了状況を保持しており、既存データの整形・表示のみで済む） | 全18ステップ完了時に、所要日数・発生した警告・KPI最終値などの要約をレポート形式で表示する。研修の振り返り資料として使える |
| 分析（KPI/原価/能力） | 表データのCSVエクスポート（新規提案） | 高（`.panel__table`の表示データをCSV化してダウンロードさせるだけの小規模機能で、複数タブで同じユーティリティを共用できる） | KPI・原価・能力タブなどの表データをCSV出力し、外部の表計算ソフトでの分析を可能にする |
| マスタ／基盤 | マスタのlocalStorage永続化 | 中（実装コスト自体は中程度だが、CLAUDE.md記載の「永続化なし・単一セッション」という設計方針そのものの転換になるため、着手前に方針変更の可否をユーザーに確認する必要がある） | ブラウザリロードでマスタ編集内容が失われる現状を、localStorageへの自動保存で解消する案。JSON入出力（`masterIO.ts`）を土台にできる |
...
| 能力（CRP） | 計画オーダ段階での山積みプレビュー | 中（design.md §9.7 C2-3で検討済みの延長。`capacity.ts`をPLANNED_ORDER段階まで対象に広げるだけで、破壊的変更は伴わない） | 現状は確定済みオーダのみが山積み計算の対象。計画オーダ確定前に負荷超過が見えれば確定判断の材料になる |
| 計画（MRP） | 複数受注の競合（v5-spec.md §11 Phase 5） | 中（design.md §6の複数受注演習＝TC-M1で基本的な資源競合とEXT-1の需要ソート順は検証済み。優先順位付けルールの明示的な切替や、より複雑な競合シナリオの追加が焦点になる） | 現状はEXT-1の固定ソート順（納期昇順優先）のみ。優先順位付けの方針自体をテーマとして扱う拡張（顧客ランク別優先度など） |
| ダッシュボード／能力 | ボトルネック作業区の推移ハイライト | 中（`dashboardHistory`と`capacity.ts`を組み合わせる新規集計が必要だが、日次スナップショットと山積み計算という既存資産を再利用できる） | 山積み超過が特定の作業区で慢性的に発生していることを日次推移で検知・ハイライトする |
| ダッシュボード／KPI | KPIのトレンド化 | 中（`dashboardHistory`に日次スナップショットが既に蓄積されているためグラフ描画の追加で実装可能。ただしKPI 12指標分のグラフ設計・配置検討には一定の工数がかかる） | 現状は数値カードのみのKPIダッシュボードを、日次推移の折れ線グラフ等へ拡張する |
...
| 基盤（品質保証） | アクセシビリティの自動テスト化（新規提案） | 中（既存のPlaywright環境に`axe-core`等を組み込む形で実現でき、`ux-reviewer`サブエージェントの手動レビューをCI上の継続監視で補完できる。ただし6テーマ×15画面の検証範囲の洗い出しに初期コストがかかる） | `ux-reviewer`が担っている手動アクセシビリティレビューの一部を、CI上の自動テストとして継続的に担保する |
